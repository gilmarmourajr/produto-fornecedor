...
public class Util {
    ...
    private void pesquisarProduto() {
        ...
        for(int i=0; i<idxProduto; i++) {
            if(produto[i].getNome().equalsIgnoreCase(nome)) {
                aux = "";
                aux += "Nome do produto: " + produto[i].getNome() + "\n";
                aux += "Valor unitário: " + fM.format(produto[i].getValorUnitario()) + "\n";
    ...
    private void pesquisarExibirFornecedor() {
        Fornecedor fornecedor = pesquisarFornecedor();
        if(fornecedor == null) {
            return;
        }

        String output = fornecedor.getNome() + ", CNPJ " +fornecedor.getCnpj() + "\n";
